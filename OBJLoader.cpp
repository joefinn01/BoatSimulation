							  ...
							  std::vector<XMFLOAT3>& outNormals)
{
	...
	for(int i = 0; i < numVertices; ++i) //For each vertex
	{
		...
		else //if not found, add it to the buffer
		{
			...
			outNormals.push_back(XMFLOAT3(vertex.Normal.x, vertex.Normal.y, vertex.Normal.z));
...
MeshData OBJLoader::Load(char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords, SphereCollider* collider)
{
	std::string binaryFilename = filename;
	binaryFilename.append("Binary");
	std::ifstream binaryInFile;
	...
	if (!binaryInFile.good())
	{
		...
		else
		{
			...
			for (unsigned int i = 0; i < numMeshVertices; ++i)
			{
				...
				float tempDistance = 0.0f;	//Calculate radius of sphere collider

				XMStoreFloat(&tempDistance, XMVectorSubtract(XMLoadFloat3(&collider->Position), XMLoadFloat3(&finalVerts[i].Pos)));

				if (collider->Radius < abs(tempDistance))
				{
					collider->Radius = abs(tempDistance);
				}
			}
			...
			outbin.write((char*)finalVerts, sizeof(SimpleVertex) * numMeshVertices);
			outbin.write((char*)indicesArray, sizeof(unsigned short) * numMeshIndices);
			outbin.write((char*)&collider->Radius, sizeof(float));
			outbin.close();
	...
	else
	{
		...
		//Read sphere collider radius
		binaryInFile.read((char*)&collider->Radius, sizeof(float));
