...
float4 PS( VS_OUTPUT input ) : SV_Target
{
 //   return Colour;
	...
	lightColour= (result.Ambient + result.Diffuse) + result.Specular;
	lightColour.a = DiffuseMtrl.a;

	for (int i = 0; i < MAX_PICKUPS; i++)
	{
		if (PickUps[i].Enabled == 1)
		{
			float distance = length(input.PosW - PickUps[i].Position);

			if (distance < 10.0f)
			{
				lightColour.xyz = lerp(float3(1.0f, 0.0f, 0.0f), lightColour.xyz, distance / 10.0f);
			}
		}
	}

	float distanceToEye = length(EyePosW - input.PosW);

	float4 finalColour = CalculateFog(lightColour, distanceToEye);
